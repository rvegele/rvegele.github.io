	...
	if (template.includes('@')) {
        ...
        break;
			case 'food':
				...
				break;
      case 'thing':
      	replacement = generateThing(parameters);
        break;
			case 'buildable':
				replacement = pickRandom('buildable');
				break;
			case 'character':
      	replacement = genChar(parameters);
...
function generateThing(parameters) {
	let make_plural = false;
	if (parameters.includes('plural')){
		make_plural = true;
	}

	if (make_plural) {
		return pluralize(pickRandom('thing'));
	} else {
		return pickRandom('thing');
	}
}

function generateFood(parameters) {
	let make_plural = false;
	if (parameters.includes('plural')){
		make_plural = true;
	}
	...
	if (make_plural) {
		return pluralize(pickRandom('food'));
	} else {
		return pickRandom('food');
	}
}
